...
<head>
    ...
    <?php
        ...
        $query = "SELECT * FROM populatie_varsta";
        $result = mysqli_query($conn, $query);
        $rows = array();
        $table = array();
        $table['cols'] = array(
            array('label' => 'Clasa de varsta', 'type' => 'string'),
            array('label' => 'Populatie', 'type' => 'number')
        );
        foreach($result as $r) {
            $temp = array();
            $temp[] = array('v' => (string) $r['varsta']);
            $temp[] = array('v' => (int) $r['populatie']);
            $rows[] = array('c' => $temp);
        }
        $table['rows'] = $rows;
        $jsonTable = json_encode($table);
        $query = "SELECT SUM(populatie) FROM populatie_varsta";
        $result = mysqli_query($conn, $query);
...
<body>
    ...
    <div class="row">
        <div class="container">
            <div class="col s12">
                <h4 class="center">Populatia in functie de clase de varsta</h4>
            </div>
            <div class="col s8 offset-s2 center">
                <p>Aceasta statistica prezinta situatia populatiei Romaniei, grupata in functie de clase de varsta. Datele au fost actualizate in anul 2014, cand s-a realizat ultimul recensamant, in care s-a inregistrat o populatie de <b><?=$sum?></b> de oameni.</p>
            </div>
        </div>
    </div>
        ...
        <div class="container">
            ...
            <div id="tabular" class="col s12">
                ...
                <table class="striped highlight centered" id="sort" class="tablesorter">
                    <thead>
                        <tr>
                            <th>Clase de varsta<i class="material-icons tiny">swap_vert</i></th>
                    ...
                    <tbody>
                        <?php
                            $conn = mysqli_connect("localhost", "root", "", "demo");
                            mysqli_select_db($conn, "populatie_varsta");
                            $query = "SELECT * FROM populatie_varsta";
                            $result = mysqli_query($conn, $query);
                            while($row = mysqli_fetch_array($result)){
                                echo "<tr>";
                                echo "<td>" . $row['varsta'] . "</td><td>" . $row['populatie'] . "</td></tr>";
                            }
                            echo "</table>";
                        ?>
                    </tbody>
