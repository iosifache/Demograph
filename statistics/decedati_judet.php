...
<head>
    ...
    <?php
        ...
        $query = "SELECT * FROM decedati_judet";
        $result = mysqli_query($conn, $query);
        $rows = array();
        $table = array();
        $table['cols'] = array(
            array('label' => 'Judet', 'type' => 'string'),
            array('label' => 'Decedati pe an', 'type' => 'number')
        );
        foreach($result as $r) {
            $temp = array();
        ...
        $query = "SELECT SUM(decedati) FROM decedati_judet";
        $result = mysqli_query($conn, $query);
...
<body>
    ...
    <div class="row">
        <div class="container">
            <div class="col s12">
                <h4 class="center">Decedati in functie de judet</h4>
            </div>
            <div class="col s8 offset-s2 center">
                <p>Aceasta statistica prezinta situatia deceselor in Romania, grupate in functie de judete. Datele au fost actualizate in anul 2014, cand s-a realizat ultimul recensamant, in care s-au inregistrat <b><?=$sum?></b> decese.</p>
            </div>
        </div>
    </div>
                ...
                <table class="striped highlight centered" id="sort" class="tablesorter">
                    <thead>
                        <tr>
                            ...
                            <th>Decedati pe an<i class="material-icons tiny">swap_vert</i></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                            $conn = mysqli_connect("localhost", "root", "", "demo");
                            mysqli_select_db($conn, "decedati_judet");
                            $query = "SELECT * FROM decedati_judet";
                            $result = mysqli_query($conn, $query);
                            while($row = mysqli_fetch_array($result)){
                                echo "<tr><td>" . $row['judet'] . "</td><td>" . $row['decedati'] . "</td></tr>";
                            }
                            echo "</table>";
                        ?>
                    </tbody>
