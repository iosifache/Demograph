...
<head>
    ...
    <?php
        ...
        $query = "SELECT * FROM nascuti_judet";
        $result = mysqli_query($conn, $query);
        $rows = array();
        $table = array();
        $table['cols'] = array(
            array('label' => 'Judet', 'type' => 'string'),
            array('label' => 'Nascuti', 'type' => 'number')
        );
        foreach($result as $r) {
            $temp = array();
            $temp[] = array('v' => (string) $r['judet']);
            $temp[] = array('v' => (int) $r['nascuti']);
            $rows[] = array('c' => $temp);
        }
        $table['rows'] = $rows;
        $jsonTable = json_encode($table);
        $query = "SELECT SUM(nascuti) FROM nascuti_judet";
        $result = mysqli_query($conn, $query);
...
<body>
    ...
    <div class="row">
        <div class="container">
            <div class="col s12">
                <h4 class="center">Nascuti in functie de judet</h4>
            </div>
            <div class="col s8 offset-s2 center">
                <p>Aceasta statistica prezinta situatia nasterilor in Romania, grupata in functie de judete. Datele au fost actualizate in anul 2014, cand s-a realizat ultimul recensamant, in care s-a inregistrat un total de <b><?=$sum?></b> copii la nivelul national.</p>
            </div>
        </div>
    </div>
            ...
            <div id="tabular" class="col s12">
                ...
                <table class="striped highlight centered" id="sort" class="tablesorter">
                    ...
                    <tbody>
                        <?php
                            $conn = mysqli_connect("localhost", "root", "", "demo");
                            mysqli_select_db($conn, "nascuti_judet");
                            $query = "SELECT * FROM nascuti_judet";
                            $result = mysqli_query($conn, $query);
                            while($row = mysqli_fetch_array($result)){
                                echo "<tr>";
                                echo "<td>" . $row['judet'] . "</td><td>" . $row['nascuti'] . "</td></tr>";
                            }
                            echo "</table>";
                        ?>
                    </tbody>
