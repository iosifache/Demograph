...
<head>
    ...
    <?php
        ...
        $query = "SELECT * FROM decedati_boala";
        $result = mysqli_query($conn, $query);
        $rows = array();
        $table = array();
        $table['cols'] = array(
            array('label' => 'Clasa de boli', 'type' => 'string'),
            array('label' => 'Decedati', 'type' => 'number')
        );
        foreach($result as $r) {
            $temp = array();
            $temp[] = array('v' => (string) $r['boala']);
            $temp[] = array('v' => (int) $r['decedati']);
            $rows[] = array('c' => $temp);
        }
        $table['rows'] = $rows;
        $jsonTable = json_encode($table);
        $query = "SELECT SUM(decedati) FROM decedati_boala";
        $result = mysqli_query($conn, $query);
...
<body>
    ...
    <div class="row">
        <div class="container">
            <div class="col s12">
                <h4 class="center">Decedati in functie de clase de boli</h4>
            </div>
            <div class="col s8 offset-s2 center">
                <p>Aceasta statistica prezinta situatia deceselor in Romania, grupate in functie de clase de boala. Datele au fost actualizate in anul 2014, cand s-a realizat ultimul recensamant, in care s-a inregistrat un total de <b><?=$sum?></b> decese, pe parcursul ultimilor 10 ani.</p>
            </div>
        </div>
    </div>
        ...
        <div class="container">
            ...
            <div id="tabular" class="col s12">
                ...
                <table class="striped highlight centered" id="sort" class="tablesorter">
                    <thead>
                        <tr>
                            <th>Clase de boli<i class="material-icons tiny">swap_vert</i></th>
                            <th>Decedati<i class="material-icons tiny">swap_vert</i></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                            $conn = mysqli_connect("localhost", "root", "", "demo");
                            mysqli_select_db($conn, "decedati_boala");
                            $query = "SELECT * FROM decedati_boala";
                            $result = mysqli_query($conn, $query);
                            while($row = mysqli_fetch_array($result)){
                                echo "<tr>";
                                echo "<td>" . $row['boala'] . "</td><td>" . $row['decedati'] . "</td></tr>";
                            }
                            echo "</table>";
                        ?>
                    </tbody>
